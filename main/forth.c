...
#define PUSHDS(X)                                                 \
    *(unsigned short *)(image->memory + GET(SPP)) = (X & 0xFFFF); \
    SET(SPP, GET(SPP) + CELL_SIZE)
#define PUSHRS(X)                                                 \
    *(unsigned short *)(image->memory + GET(RPP)) = ((X)&0xFFFF); \
    SET(RPP, GET(RPP) - CELL_SIZE)
#define POPDS(X)                    \
    SET(SPP, GET(SPP) - CELL_SIZE); \
    X = *(unsigned short *)(image->memory + GET(SPP))
#define POPRS(X)                    \
    SET(RPP, GET(RPP) + CELL_SIZE); \
    X = *(unsigned short *)(image->memory + GET(RPP))
                ...
                unsigned short type, unsigned short data)
{
    ...
    while (x < size)
    {
        image->memory[charstart + x] = toupper(word[x]);
        x++;
    }
    if ((size) % 2 == 1)
        image->memory[charstart + size] = '\0';
...
int int_find(struct IMAGE *image, char *word, int size)
{
    ...
    while (start != 0)
    {
        if (len == size)
        {
            ...
            while (x < len)
            {
                match &= toupper((word[x]) == toupper(image->memory[start + 1 + x]));
                x++;
            }
            if (match > FALSE)
                return start;
